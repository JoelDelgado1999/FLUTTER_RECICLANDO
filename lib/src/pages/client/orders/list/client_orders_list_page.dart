...
import 'package:flutter_recolectando_innomine/src/pages/client/orders/list/client_orders_list_controller.dart';
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      alignment: Alignment.centerLeft,
                      child: Text(
                          'RECOLECTOR: ${order.delivery?.name ?? 'No asignado'} ${order.delivery?.lastname ?? ''}'),
                    ),
                    Container(
                      width: double.infinity,
                      margin: EdgeInsets.only(top: 5),
                      ...
                      builder: (context, AsyncSnapshot<List<Order>> snapshot) {
                        if (snapshot.hasData) {
                          if (snapshot.data!.length > 0) {
                            ...
                            return Center(
                                child:
                                    NoDataWidget(text: ' No hay solicitudes'));
                          }
                        } else {
                          return Center(
                              child: NoDataWidget(text: 'No hay solicitudes'));
                        }
                      });
                }).toList(),
              )),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      alignment: Alignment.centerLeft,
                      child: Text(
                          'Direccion de recoleccion: ${order.address?.address ?? ''}'),
                    ),
                    Container(
                        width: double.infinity,
                        margin: EdgeInsets.only(top: 5),
                        alignment: Alignment.centerLeft,
                        child: Text(
                            'Nombre: ${order.nombre_evidencia ?? 'nos asissgnado'}')),
                  ],
                ),
              ),
            ],
